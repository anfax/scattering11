...
!
!***********************************************************************
 	program cdc4   ! Version 4n    including core deformation
...
!	open(ko3,recl=80,form='formatted',delim='apostrophe')
	...
	write(0,*) 'CDC: Fresco front end for CDCC, version cdc4n'
	write(0,*) 
	eps = 1d-8
      	read(ki,1005) headng
...
c correction of error for E binning, see below
             bhat(ib) = bstart(ib)+step/2
          else
	     xk = kstart + (ie-1)*kstep
	     bstart(ib) = xk**2/c1
             bend(ib) = (xk + kstep)**2/c1
c correction of error for k binning, see below
             bhat(ib) = ( (xk+0.5*kstep)**2 + kstep**2/12.)/c1 * bound
          endif
c old version, probably error, should only be for k-binning, not E-binning
c          bhat(ib) = ( (xk+0.5*kstep)**2 + kstep**2/12.)/c1 * bound
