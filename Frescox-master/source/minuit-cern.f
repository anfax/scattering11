...
* under the terms of the GNU General Public License as at
* http://www.gnu.org/copyleft/gpl.html
* 
* The software and documentation made available under 
* the terms of this license are provided with no warranty.
...
C
     ...
     */MN7XCR/ XMIDCR ,YMIDCR ,XDIRCR ,YDIRCR ,KE1CR  ,KE2CR
      CHARACTER CTITL*50, CWORD*(MAXCWD), CUNDEF*15, CFROM*8,
     *          CVRSN*6,  COVMES(0:3)*22, CSTATU*10, CHPT*1
      LOGICAL   LWARN, LREPOR, LIMSET, LNOLIM, LNEWMN, LPHEAD


C
C  CPNAM   Parameter name (15 characters)
...
C                    IZ is number of columns to be printed in row I
      ...
      DO 160 I= 1, NPARD
         ...
         DO 150 K= 1, IZ, NPERLN
           K2 = K + NPERLN - 1
           IF (K2 .GT. IZ)  K2=IZ
           WRITE (ISYSWR,'(1X,1p,13E10.3)')  (EMAT(I,KK),KK=K,K2)
  150    CONTINUE
  160 CONTINUE
      ENDIF
      RETURN
...
C         if different from internal, external values are taken
  ...
  520 FORMAT (20X, 9HPARAMETER,I4,2H, ,A15,' RESTORED TO VARIABLE.')
      END
*
...
C                    get MINUIT-calculated first derivatives
   ...
   99 FORMAT (7X,I5,2X ,A15,3E12.4,4X ,A4)
  100 CONTINUE
...
C         used by MNPINT to set variables "near" the physical limits
      ...
      VLIMLO = -PIBY2 + DISTNN
      CALL MNCLER
      WRITE (ISYSWR,'(3A,I4,A,I4,A,E10.2)')  '  MINUIT RELEASE ',CVRSN,
...
C                                        . . .constant parameter . . . .
   ...
   82 FORMAT (1X,I5,1X,1H',A15,1H',1X,G13.5, '  constant')
      NVL = 0
      GO TO 200
...
C                                      variable parameter without limits
  ...
  127 FORMAT (1X,I5,1X,1H',A15,1H',1X,2G13.5, '     no limits')
      ELSE
C                                         variable parameter with limits
      NVL = 4
      LNOLIM = .FALSE.
      IF (ISW(5) .GE. 0)  WRITE (ISYSWR, 132)  K,CNAMK,UK,WK,A,B
  132 FORMAT(1X,I5,1X,1H',A15,1H',1X,2G13.5,2X,2G13.5)
      ENDIF
...
C          old (fixed-field) format
  ...
  158 FORMAT (BN,F10.0, A15, 4F10.0)
      K = XK
      IF (K .EQ. 0)  GO TO 210
...
C              variable parameter.
  ...
  952 FORMAT (I4,1X,A15,2G14.5,2A)
...
C                                print constant or fixed parameter.
   55 CONTINUE
      ...
      IF (NVARL(I).EQ.4 .AND. IKODE.EQ.1) THEN
        WRITE (ISYSWR,'(I4,1X,A15,G14.5,A,2G14.5)')
     *     I,CNAMBF,U(I),COLHDU(1),ALIM(I),BLIM(I)
      ELSE
        WRITE (ISYSWR,'(I4,1X,A15,G14.5,A)')  I,CNAMBF,U(I),COLHDU(1)
      ENDIF
  200 CONTINUE
C
...
C         parameter with limits
 ...
 1001 FORMAT (1X,I5,1H',A15,1H',4E13.5)
  200 CONTINUE
      WRITE (ISYSSA,'(A)')  ' '
      NLINES = NLINES + 1
...
C         finished with all parameters
 ...
 1001 FORMAT (I1,'SCAN OF PARAMETER NO.',I3,3H,   ,A15)
      END
*
