...
*****POTENT*************************************************************
	...
	use io
	use searchpar
	use fresco1, only: pluto,npluto,ppots,nextpot
...
C    9  SHAPE  Mr    Mrr0  Mra                     1 - Effective-mass
...
C
C   24  SHAPE  Vr    Vrr0  Vra   Vi    Vir0  Via   Central Volume  for (N-Z)/A Lane diagonal		
C   25  SHAPE  Wr    Wrr0  Wra   Wi    Wir0  Wia   Central Derivative  for (N-Z)/A Lane diagonal	
C   26  SHAPE  Vr    Vrr0  Vra   Vi    Vir0  Via   Central Volume  for (N-Z)/A Lane coupling 		
C   27  SHAPE  Wr    Wrr0  Wra   Wi    Wir0  Wia   Central Derivative  for (N-Z)/A Lane coupling 	
C
C   30  SHAPE  VLL   VLLr0 VLLa  WLL   WLLr0 WLLa  Central L(L+1) 
C
C If -9 < TYPE < 0 or   -27 <= TYPE <= -24,
C		    then merge with the previous form,
...
C           10-19: write SHAPE-10 to file 25 (TYPE 1 to 9 only)
...
C           43:  use potential P1 for even L, P2 for odd L
C           45:  linearly interpolate potential P1 at energy P2, P3 at E=P4, P5 at E=P6
C           -1:  Fourier-Bessel shape
C        <= -2:  <new shapes go here>
C
C      for TYPE = 1-9 or 24-27 or 30 (incl), must have  -1 <= SHAPE <= 9
...
6     CONTINUE
	...
	do i=1,npluto
	 ...
	 if(pluto(i)>20) then
 		write(0,*) ' All PLUTO potentials <= 20 please'
		stop
	  endif
	enddo
	nextpot(:) = 1
...
!12    FORMAT(I3,I2,L1,I2,7F8.4)
      ...
      KP = ABS(KPI)
	if(KPOP(KP)==0) then
	 ppots(nextpot(KP),KP) = '&pluto'
	 write(ppots(nextpot(KP)+1,KP),880) HCM,(N-1)*HCM
880	 format('  frescocall=T   h=',f7.3,' rmax=',f7.2)
         nextpot(KP) = nextpot(KP)+2  ! next available line for this potential
	 KPOP(KP) = 1
	endif
...
!                                any linked search variables?
          if(srch_pline2(ip)*srch_col2(ip)>0) then
            if(line(ip)==srch_pline2(ip)) then
                DEF(srch_col2(ip)) = srch_value(ip)*srch_ratio2(ip)
            endif
          endif
...
C                STOP 'POTENTIALS'
 ...
 1202  CONTINUE
	...
	HASO(KP) = HASO(KP).or.(TYPEI==3.and.abs(P1)+abs(P4)>1e-9)
	HASO(KP) = .false.   !! NOT PROPERLY IMPLEMENTED YET !!
...
C                        remove next line when Coul quadrature written.
	       ...
	       LIN =KPOP(KP)
	       if(LIN>=0) then
	        write(ppots(nextpot(KP),KP),890) LIN,P1,P2,P3,LIN,
     x                                             TYPE,LIN,SHAPE
	        nextpot(KP) = nextpot(KP)+1  ! next available line for this potential
...
!				 any linked search variables?
		   if(srch_pline2(ip)*srch_col2(ip)>0) then
	 	   if(line(ip)==srch_pline2(ip)) then
		    str = srch_value(ip)*srch_ratio2(ip)
		   endif
		   endif
		endif
		enddo
...
C					The monopole is replaced completely!
	...
	if(TYPE==26 .or. TYPE==27) lanecoup=.true.
C
      IF(TYPE.EQ.0) THEN
         PT(2) = ABS(P1) .GT. 1E-5
         PT(1) = ABS(P2) .GT. 1E-5
	...
	if(TYPE==0) then
	 ...
	 write(ppots(nextpot(KP),KP),  888) P1,P2
	 write(ppots(nextpot(KP)+1,KP),889) LIN,P3,P4,LIN,TYPE,LIN,SHAPE
888	 format('  a2=',f8.3,' a1=',f8.3)
889	 format('  potl(3:4,',i3,') =',2f12.5,12x,' potltype(',i3,')=',i2,
     &    '  potlshape(',I3,')=',I2)
                nextpot(KP) = nextpot(KP)+2  ! next available line for this potential
	else
	 write(ppots(nextpot(KP),KP),890) LIN,P1,P2,P3,LIN,TYPE,LIN,SHAPE
...
18       WRITE(KO,173) KP,TYPE,WHO(TYPE+1),K,NNKIND(K),M
	...
	STOP19 = .true.
	  ...
	  enddo
	  FORMDEF(1:12,NF) = DEF(1:12)
        WRITE(KO,178) KP,TYPE,WHO(TYPE+1),cit,SHAPE,SHPNAM,NF,
     X         (DEF(K),K=1,J)
...
C
      ...
      PTYPE(3,M) = 0
         ...
         IF(TYPE>=5 .and. TYPE<=7) PTYPE(3,M) = 2
      PTYPE(4,M) = 0
      PTYPE(5,M) = 0
...
195   IF(SHAPE.GE.7.AND.SHAPE.LE.9) GO TO 70
         IF(TYPE.EQ.0) THEN
            RR = P3 * CC
            GO TO 100
         ELSE IF(TYPE.LE.9.or.TYPE.ge.17) THEN
...
C
      ...
      INTEGER PTYPE(12,MLOC),LAMBDA(MLOC)
      LOGICAL QUAD,VOLCON
