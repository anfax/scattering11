...
C
     ...
     X          R0,R1,R2,R3,R4,R
23        FORMAT('0KERNEM:',2i6,4I3,5F8.4,F9.6)
...
100   RETURN
      END
      SUBROUTINE QERNEM(NLN,NLM,NLO,NK,NG,XA,XB,XP,XQ,FPT,CUTOFF,
     ...
     X             MXMV1,MXMVP1,MXLN1,MXLNP1,FORMC,VSC)
	use parameters
	use io
	use drier
      IMPLICIT REAL*8(A-H,O-Z)
     ...
     X       VSP(MAXNLR,MSP),FORML(MAXNLR,MSP),
