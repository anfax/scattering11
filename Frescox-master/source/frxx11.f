...
C                                  make arrays for CRCWFN
	   ...
	   if(SHOW>2) then
            if(DERIV) write(6,*) 'PWISER: DERIV at ',RIN(1)
            if(.not.DERIV) write(6,*) 'PWISER: FNS at ',RIN(1:2)
	   endif
...
c
               ...
               if (tmp1.le.0.d0.or.tmp2.le.0.d0) then
                    write(KO,*) 'Problem 1 with CRCWFN match to zero',
     *                           tmp1,tmp2
                    tmp6 =1.5d0
                    goto 778 
               endif
...
c
               if (tmp3.le.0.d0.or.abs(tmp5).gt.1.d0) then
                    write(KO,*) 'Problem 2 with CRCWFN match to zero',
     *                           tmp3,tmp5
                    tmp6 =1.5d0
                    goto 778 
               endif
...
C
	   ...
	   J1=C1-JST
	   J2=C2-JST
	write(KO,20) C1,C2,(COUPL(J1,J2,I),I=1,KMAX)
20	format(' PW: COUPL(',i3,',',i3,') =',10f12.5)
	   enddo
	   enddo
	  endif
C
